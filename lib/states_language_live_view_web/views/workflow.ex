defmodule StatesLanguageLiveViewWeb.Workflow do
  ...
  @doc """
  Once our LiveView socket is connected, we'll start our ExampleWorkflow. This module is our state machine implementation.

  We pass a struct of type ExampleWorkflow as the data for our state machine. This will get populated with data as we move through our state machine.
  ...
  @doc """
  When inputting a phone number or account id in our HTML form, and pressing the button, this callback pattern matches on our event type, and uses the remainder of the binary data to indicate which form name to get our data from. In this case it also happens to match the key we're going to store the value in, in our state machine data.
  """
  ...
  @doc """
  This is a GenServer callback that matches on messages sent from our state machine. It updates our data attribute on our socket, which triggers LiveView to push a message down the WebSocket, and re-render our form.
  """
